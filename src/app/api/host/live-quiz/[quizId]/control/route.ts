...
export async function POST(
  ...
  return handleApi(async () => {
    ...
    const hostUser = await requireQuizHost();
    ...
    if (hostUser.role === "quiz_host" && quizData.hostUid !== hostUser.uid) {
      throw new ApiError(403, "You are not authorized to host this quiz.");
    }
    ...
    await audit({
      actorUid: hostUser.uid,
      actorEmail: hostUser.email,
      action: `host_live_quiz.${action}`,
      target: quizId,
